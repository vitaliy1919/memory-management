...
public class Kernel extends Thread
{
  ...
  public int runs;
  public PageFault pageFault = new PageFault();
  public int runcycles;
  public long workingSetTime = 40;
  public long maxDisksWrites = 5;
  ...
  public void init( String commands , String config )  
  {
        ...
        while ((line = in.readLine()) != null) 
        {
          ...
          { 
            ...
            while (st.hasMoreTokens()) 
            {
              ...
              {
                System.out.println("com.MemoryManagement: numpages out of bounds.");
                System.exit(-1);
              }
...

        {
          ...
          { 
            ...
            { 
              ...
              {
                System.out.println("com.MemoryManagement: Invalid page value in " + config);
                System.exit(-1);
              }
              R = Common.s2b(st.nextToken());
              if (R < 0 || R > 1)
              {
                System.out.println("com.MemoryManagement: Invalid R value in " + config);
                System.exit(-1);
              }
              M = Common.s2b(st.nextToken());
              if (M < 0 || M > 1)
              {
                 System.out.println("com.MemoryManagement: Invalid M value in " + config);
                 System.exit(-1);
              }
              inMemTime = Common.s2i(st.nextToken());
              if (inMemTime < 0)
              {
                System.out.println("com.MemoryManagement: Invalid inMemTime in " + config);
                System.exit(-1);
              }
              lastTouchTime = Common.s2i(st.nextToken());
              if (lastTouchTime < 0)
              {
                System.out.println("com.MemoryManagement: Invalid lastTouchTime in " + config);
                System.exit(-1);
              }
          ...
          if (line.startsWith("working_set_time")) {
            StringTokenizer st = new StringTokenizer(line);
            if (st.hasMoreTokens())
            {
              st.nextToken();
              workingSetTime = Common.s2i(st.nextToken());
              if (workingSetTime % 10 != 0 || workingSetTime < 10)
              {
                System.out.println("com.MemoryManagement: Invalid workingSetTime in " + config);
                System.exit(-1);
              }
            }
          }
          if (line.startsWith("max_disk_writes")) {
            StringTokenizer st = new StringTokenizer(line);
              st.nextToken();

              if (st.hasMoreTokens())
            {
              maxDisksWrites = Common.s2i(st.nextToken());
              if (maxDisksWrites <= 0  || maxDisksWrites > 32)
              {
                System.out.println("com.MemoryManagement: Invalid maxDisksWrites in " + config);
                System.exit(-1);
              }
            }
          }
          ...
          { 
            ...
            {
              System.out.println("com.MemoryManagement: pagesize is out of bounds");
              System.exit(-1);
            }
            for (i = 0; i <= virtPageNum; i++) 
            {
          ...
          { 
            ...
            while (st.hasMoreTokens()) 
            {
              ...
              {
                System.out.println("com.MemoryManagement: addressradix out of bounds.");
                System.exit(-1);
              }
            }
          }
      ...
      while ((line = in.readLine()) != null) 
      {
        if (line.startsWith("READ") || line.startsWith("WRITE")) 
        {
          ...
          { 
            ...
            {
              System.out.println("com.MemoryManagement: " + addr + ", Address out of range in " + commands);
              System.exit(-1);
            }
    ...
    if ( runcycles < 1 )
    {
      System.out.println("com.MemoryManagement: no instructions present for execution.");
      System.exit(-1);
    }
    if ( doFileLog )
    {
    ...
    for (i = 0; i < virtPageNum; i++) 
    {
      ...
      if (physical_count > 1)
      {
        System.out.println("com.MemoryManagement: Duplicate physical page's in " + config);
        System.exit(-1);
      }
      physical_count = 0;
    }
    ...
    for (i = 0; i < instructVector.size(); i++) 
    {
      ...
      {
        System.out.println("com.MemoryManagement: com.Instruction (" + instruct.inst + " " + instruct.addr + ") out of bounds.");
        System.exit(-1);
      }
    }
  } 
